...
import DifferentProfile, {
  type DifferentProfileItem,
} from "../../components/profile/DifferentProfile";
...
export default function CreateEvaluations({
  ...
  const ingredients = React.useMemo<IngredientRow[]>(() => {
    ...
    if (allIngredients.length > 0) {
      return allIngredients.slice(0, 6).map((name) => {
        ...
        const hasAllergen = [...matchedAllergens].some((item) =>
          lower.includes(item.toLowerCase()),
        );
        const hasCondition = [...matchedConditions].some((item) =>
          lower.includes(item.toLowerCase()),
        );
  ...
  const score =
    typeof resultJson?.score === "number"
      ? Math.max(0, Math.min(100, resultJson.score))
      : null;
  const status = resultJson?.status;
  const statusTone =
    status && ["safe", "caution", "avoid"].includes(status) ? status : null;
  ...
  const activeProfileId =
    activeDifferentProfileId ?? profileSwitcherItems[0]?.id;
    ...
    <Box style={styles.screen}>
      ...
      >
        ...
        <DangerousIngredients
          items={resultJson?.dangerous_ingredients}
          index={2}
        />
        ...
        <AllIngredients
          items={toStringArray(resultJson?.all_ingredients)}
          index={5}
        />
        ...
        {additionalEntries.length > 0 ? (
          ...
          >
            {additionalEntries.map(([key, value], index) => (
              <Box
                key={key}
                mb={index === additionalEntries.length - 1 ? 0 : 10}
              >
                <Text
                  fontSize={12}
                  lineHeight={16}
