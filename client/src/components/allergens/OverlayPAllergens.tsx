...
export default function OverlayPAllergens({
	isOpen,
	onClose,
	allergens,
	selectedAllergenIds,
	onSave,
}: OverlayPAllergensProps) {
	...
	React.useEffect(() => {
		// Reset draft selection whenever the modal opens so edits start from current saved data.
		if (isOpen) {
			setDraftSelectedIds(selectedAllergenIds);
			setIsSaving(false);
			setIsClosing(false);
		}
	...
	React.useEffect(() => {
		// Prevent delayed close timer leaks when component unmounts.
		return () => {
	...
	const selectedAllergens = React.useMemo(
		() => allergens.filter((item) => draftSelectedIds.includes(item.id)),
		[allergens, draftSelectedIds]
	);

	const toggleAllergen = (allergenId: string) => {
		// Toggle add/remove in local draft; persisted only when Save is pressed.
	...
	const triggerClose = React.useCallback(() => {
		...
		// Keep close timing aligned with the exit animation for a smoother dismiss.
		setIsClosing(true);
	...
	const handleSave = async () => {
		try {
			// Persist the draft selection back to parent state, then close.
			setIsSaving(true);
